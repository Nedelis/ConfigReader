...
____
#### 3. Комментарии.
По умолчанию комментарии работают только в начале строки. На данный момент существую только комментарии-строки. Префикс для комментария по умолчанию — `//`. Вы можете изменить его в файле `reader.properties`, который генерируется по указанному вами пути.
____
#### 4. Секции.
...
Обратите внимание, что при указании в одном файле нескольких модификационных имён разных парсеров/транслейтеров, будет выбран тот, чья модификация идёт первой. Добавлять свои модификации можно только регистрируя новый парсер/транслейтор.
____
